...
var DEVICE_COUNT = (argv.count ? argv.count : (process.env.DEVICE_COUNT || 1));
var TELEMETRY_RATE = (argv.rate ? argv.rate : (process.env.TELEMETRY_RATE || 2));

console.log("Simulating " + DEVICE_COUNT + " phones");
...
var quito_map = new Map("quito", "QUITO", "maps/Quito.osm");

function Device(id, suffix) {
	this.id = id;
	this.suffix = suffix;
	this.name = "Dispositivo: " + id + suffix;
	this.map = quito_map;
	this.speed = null;
	this.state = "normal";
...
Device.prototype.drive = function() {
	this.frame++;
	if (!this.currentEdge) {
...
Device.prototype.update = function() {
	...
	//console.log("updating device: distance remaining = " + dist_to_move);
...
Device.prototype.getPublishPayload = function() {
	return {
...
Device.prototype.setSpeed = function(val) {
	// speed = km/hr
	if (val >= 0 && val <= 120) {
...
Device.prototype.setTurnSignal = function(val) {
	if (val == "LEFT" || val == "NONE" || val == "RIGHT" || val == "ON") {
		this.customProps.turnSignal = val;
	}
}

Device.prototype.setStartingEdge = function() {
	var keys = [];
...
Device.prototype.updatePosition = function() {
...
Device.prototype.setNextEdge = function() {
...
var devices = Array();
for (var i = 1; i <= DEVICE_COUNT; i++) {
	var vid = deviceId;
	var suffix = "";
	if (DEVICE_COUNT > 1) { suffix = "-" + i; }
	devices.push(new Device(vid, suffix));
}
...
function publishDeviceData() {
	var payload = [];
	for (var i in devices) {
		payload.push(devices[i].getPublishPayload());
	}
...
function getDevice(id) {
	for (var i in devices) {
		if ((devices[i].id + devices[i].suffix) == id) {
			return devices[i];
		}
	}
	return null;
				...
				"regions" : [
					{
						...
						"notifyOnExit": "true",
						"polygon": [
							{"latitude": "-0.194974", "longitude": "-78.4799179"},
							{"latitude": "-0.195974", "longitude": "-78.4804179"},
							{"latitude": "-0.196974", "longitude": "-78.4814179"},
							{"latitude": "-0.197974", "longitude": "-78.4829179"}

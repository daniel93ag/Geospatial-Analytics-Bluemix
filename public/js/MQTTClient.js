...
function defineSubs() {
	...
	Subscriptions.geoAlerts = new Subscription(window.config.notifyTopic, function(msg) {
		console.log("notifyTopic: "+window.config.notifyTopic);
		...
		try {
			...
			var tipo="";
			if(text=="Entry")
				tipo="Entró a la zona: ";
			if(text=="Exit")
				tipo="Salió de la zona: ";
			//alert(id+" "+tipo+data.regionId);
			console.log("El dispositivo: "+id+" "+tipo+data.regionId);
